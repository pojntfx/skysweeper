...
make -j$(nproc) depend
go run ./cmd/aeolius-server manager --origin http://localhost:3000
...
go run ./cmd/aeolius-server worker --api-key "${API_KEY}" # Append --dry-run=false to actually delete posts instead of just logging the execution plan
